...
const p = new Pool({
  connectionString: 'postgresql://postgres.jnoqtchsrartktaxwcdu:Ephreme0977%40@aws-1-eu-west-1.pooler.supabase.com:6543/postgres',
  ssl: { rejectUnauthorized: false }
})

p.query('SELECT id, full_name, email, role, status FROM public.users WHERE email = $1', ['admin@motaed.cd'])
  .then(r => {
    console.log('public.users count:', r.rows.length)
    console.log(r.rows)
  ...
  .then(r2 => {
    console.log('auth.users count:', r2.rows.length)
    console.log(r2.rows)
    return p.end()
  })
  .catch(e => {
